...
# def get_mamujoco_yaml_args(algo, env, task, agent_conf):
...
#
#     env_args["scenario"] = task
#     env_args["agent_conf"] = agent_conf
...
def get_defaults_yaml_args(args):
    ...
    with open(env_cfg_path, "r", encoding="utf-8") as file:
        env_args = yaml.load(file, Loader=yaml.FullLoader)
    if env == "football":
        env_args["env_name"] = args["task"]
        env_args["number_of_left_players_agent_controls"] = args["agent_f"]
    elif env == "smac":
        env_args["map_name"] = args["task"]
    elif env == "smacv2":
        env_args["map_name"] = args["task"]
    elif env == "mamujoco":
        env_args["scenario"] = args["task"]
        env_args["agent_conf"] = args["agent"]
    elif env == "lag":
        env_args["scenario"] = args["task"]
...
def get_smac_yaml_args(algo, env, task):
    """Load config file for user-specified algo and env.
    Args:
        algo: (str) Algorithm name.
        env: (str) Environment name.
    ...
    """
    base_path = os.path.split(os.path.dirname(os.path.abspath(__file__)))[0]
    algo_cfg_path = os.path.join(base_path, "configs_smac", "algos_cfgs", f"{algo}.yaml")
    env_cfg_path = os.path.join(base_path, "configs_smac", "envs_cfgs", f"{env}.yaml")

    with open(algo_cfg_path, "r", encoding="utf-8") as file:
        algo_args = yaml.load(file, Loader=yaml.FullLoader)
    with open(env_cfg_path, "r", encoding="utf-8") as file:
        env_args = yaml.load(file, Loader=yaml.FullLoader)
    env_args["map_name"] = task
    return algo_args, env_args
