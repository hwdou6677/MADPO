...
class OnPolicyHARunner(OnPolicyBaseRunner):
    ...
    def train(self):
        ...
        for agent_id in agent_order:
            ...
            if self.use_div:
                if agent_id == 0:
                    actor_train_info = self.actor[agent_id].train_(
                        self.actor_buffer[agent_id],
                        self.old_actor_buffer[agent_id]
                        if len(self.old_actor_buffer) != 0
                        else self.actor_buffer[agent_id],
                        self.actor_buffer[agent_id],
                        self.actor[agent_id],
                        advantages.copy(), self.state_type
                    )
                else:
                    actor_train_info = self.actor[agent_id].train_(
                        self.actor_buffer[agent_id],
                        ...
                        self.actor_buffer[agent_id - 1],
                        self.actor[agent_id - 1],
                        advantages.copy(), self.state_type
                    )
