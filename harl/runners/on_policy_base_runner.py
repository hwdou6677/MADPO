...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        # self.special_order = algo_args["algo"]["special_order"]
        if self.args["algo"] == "madpo":
            self.use_div = algo_args["algo"]["use_div"]
        else:
            self.use_div = False
        set_seed(algo_args["seed"])
        self.device = init_device(algo_args["device"])
        # self.render_log = self.algo_args["render"]["use_render_log"]
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            ...
            for agent_id in range(self.num_agents):
                ...
                ac_bu_old_init = OnPolicyActorBuffer(
                    {**algo_args["train"], **algo_args["model"]},
                    self.envs.observation_space[agent_id],
                    self.envs.action_space[agent_id],
                )
                self.old_actor_buffer.append(ac_bu_old_init)
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            self.old_actor_buffer = self.actor_buffer.copy()
    ...
    def render(self):
        ...
        if self.manual_expand_dims:
            ...
            for i in range(self.algo_args["render"]["render_episodes"]):
                if self.render_log:
                    store_dir = self.algo_args["train"]["model_dir"]
                    actions_file = open(os.path.join(store_dir, os.pardir, str(i) + "_actions.txt"), "w",
                                        encoding="utf-8")
                ...
                while True:
                    ...
                    # print(eval_actions)
                    if self.render_log:
                        actions_file.write(", ".join(map(str, ["agent0", eval_actions[0][0][0],
                                                               "agent1", eval_actions[0][1][0],
                                                               "agent2", eval_actions[0][2][0], ])) + "\n")
