...
class OffPolicyBaseRunner:
    ...
    def run(self):
        ...
        for step in range(1, steps + 1):
            ...
            # raise
            if step > 1:
                div_act_new = self.get_actions(div_obs_pri, available_actions=div_act_avail_pri, add_random=True)
            (
                new_obs,
                new_share_obs,
                rewards,
            ...
            div_obs_pri_temp = obs.copy()
            div_act_pri_temp = actions.copy()
            div_act_avail_temp = available_actions.copy()
            ...
            if step % self.algo_args["train"]["train_interval"] == 0:
                ...
                for _ in range(update_num):
                    # if step == 1:
                    #     div_obs_pri_temp = div_obs_pri_temp.transpose(1, 0, 2)
                    #     div_act_pri_temp = div_act_pri_temp.transpose(1, 0, 2)
                    #     self.train(div_obs_pri_temp, div_act_pri_temp, div_act_pri_temp)
                    # else:
                    #     # pass
                    div_obs_pri = div_obs_pri.transpose(1, 0, 2)
                    div_act_pri = div_act_pri.transpose(1, 0, 2)
                    div_act_new = div_act_new.transpose(1, 0, 2)
                    # print(div_act_pri, div_obs_pri, div_act_new)

                    self.train(div_obs_pri, div_act_pri, div_act_new)

            div_obs_pri = div_obs_pri_temp.copy()
            div_act_pri = div_act_pri_temp.copy()
            div_act_avail_pri = div_act_avail_temp.copy()
    ...
    def get_actions(self, obs, available_actions=None, add_random=True):
        ...
        """
        if self.args["algo"] == "hasac" or self.args["algo"] == "madac":
            actions = []
            for agent_id in range(self.num_agents):
                if (
