...
def main(algo):
    ...
    if args["load_config"] != "":  # load config from existing config file
        ...
        algo_args, env_args = get_defaults_yaml_args(args)

    update_args(unparsed_dict, algo_args, env_args)  # update args from command line
    args["seed"] = algo_args["seed"]["seed"]
    ...
    if args["env"] == "dexhands":
        ...
        env_args["device"] = algo_args["device"]["gpu"]
    ...
    if args["env"] == "smacv2":
        print(env_args["map_name"])

    print("Preparing for training %s on %s task of %s environment" % (args["algo"], args["task"], args["env"]))
    # print("hello")
...
if __name__ == "__main__":
    # algo_list_1 = ["hatrpo_CS", "hatrpo"]
    # algo_list_2 = ["happo", "mappo"]
    # for i in algo_list_1 + algo_list_2:
    #     main(i)
